...
VITAL_PATTERNS = {
    ...
    'temp': r'(?:Temp|Temperature)[:\s]*(\d+\.?\d*)\s*(?:F|C|f|c)'
}
...
def extract_vitals_from_note(note_text, patient_id, encounter_id):
    ...
    if weight_kg:
        ...
        vitals.append({
            ...
            'CODE': VITAL_LOINC_CODES['Body Weight'],
            'DESCRIPTION': 'Body Weight',
            'VALUE': round(float(weight_lb.group(1)) * 0.453592, 1),
            'UNITS': 'kg'
        })
    ...
    # Temperature
    if VITAL_PATTERNS['temp']:
        temp_match = re.search(VITAL_PATTERNS['temp'], note_text, re.IGNORECASE)
        if temp_match:
            vitals.append({
                'PATIENT': patient_id,
                'ENCOUNTER': encounter_id,
    ...
    for keyword, description in KNOWN_MEDICATIONS.items():
        if keyword in note_lower and description not in found_meds:
            ...
            medications.append({
                ...
                'ENCOUNTER': encounter_id,
                'DESCRIPTION': description,
                'REASONDESCRIPTION': 'Extracted from clinical note'
            })
