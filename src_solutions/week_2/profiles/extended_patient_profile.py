...
class ExtendedPatientProfile(PatientProfile):
    ...
    # ──────────────────────────────────────────────────────────────
    # HOMEWORK: 8 feature methods (solutions)
    # ──────────────────────────────────────────────────────────────
    ...
    def _calculate_hba1c_trend(self, target_date: datetime, window_days: int = 180) -> float:
        ...
        readings_sorted = sorted(readings, key=lambda x: safe_parse_date(x['date']))
        first_val = readings_sorted[0]['value']
        last_val = readings_sorted[-1]['value']
    ...
    def _get_bmi_category(self, target_date: datetime) -> Optional[int]:
        """
        Get BMI category from most recent BMI reading.
        Returns: 1=underweight, 2=normal, 3=overweight, 4=obese, None if no data
        """
        readings_before = [
            r for r in self.bmi_timeline
            if safe_parse_date(r['date']) and safe_parse_date(r['date']) <= target_date
        ]
        if not readings_before:
            return None
    ...
    def _get_active_medication_count(self, target_date: datetime) -> int:
        ...
        """
        # Group events by medication name
        med_events = {}
        for m in self.medication_timeline:
            m_date = safe_parse_date(m['date'])
            if not m_date:
                continue
            med_name = m.get('medication', '')
            if med_name not in med_events:
                med_events[med_name] = {'start': None, 'stop': None}
            if m.get('action') == 'started' and m_date <= target_date:
                # Keep the most recent start before target_date
                if med_events[med_name]['start'] is None or m_date >= med_events[med_name]['start']:
                    med_events[med_name]['start'] = m_date
            elif m.get('action') == 'stopped':
                # Keep the most recent stop
                if med_events[med_name]['stop'] is None or m_date >= med_events[med_name]['stop']:
                    med_events[med_name]['stop'] = m_date

        count = 0
        for med_name, dates in med_events.items():
            if dates['start'] is None or dates['start'] > target_date:
                continue
            # Active if no stop date, or stop is after target_date
            if dates['stop'] is None or dates['stop'] > target_date:
                count += 1
        return count
    ...
    def _get_longest_care_gap(self, target_date: datetime) -> Optional[int]:
        ...
        dates = sorted([safe_parse_date(r['date']) for r in readings_before])
    ...
    def get_homework_features(self, date: str) -> Dict:
        """
        Get only the 8 homework features (for testing implementations).
